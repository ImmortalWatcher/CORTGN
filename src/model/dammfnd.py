...
_DEBUG_SHAPES = os.environ.get("DAMMFND_DEBUG_SHAPES", "").strip() in ("1", "true", "yes")


def _dbg_shape(name: str, tensor) -> None:
    if _DEBUG_SHAPES and tensor is not None:
        print(f"[DEBUG shapes] {name}: {tuple(tensor.shape)}")
...
from utils.clip_load import load_cn_clip
...
class DAMMFNDMODEL(torch.nn.Module):
    def __init__(self, emb_dim, mlp_dims, bert, out_channels, dropout):
        ...
        _mae_ckpt = os.path.join(_src, "mae_pretrain_vit_{}.pth".format(self.model_size))
        checkpoint = torch.load(_mae_ckpt, map_location="cpu", weights_only=False)
        self.image_model.load_state_dict(checkpoint['model'], strict=False)
        for param in self.image_model.parameters():
            param.requires_grad = False


        self.ClipModel, _ = load_cn_clip("cuda")
    ...
    def forward(self, **kwargs):
        ...
        text_feature = self.bert(inputs, attention_mask=masks)[0]  # ([B, 197, 768])
        _dbg_shape("inputs (token ids)", inputs)
        _dbg_shape("text_feature (BERT)", text_feature)
        image = kwargs['image']
        image_feature = self.image_model.forward_ying(image)  # ([B, 197, 768])
        _dbg_shape("image (pixels)", image)
        _dbg_shape("image_feature (MAE)", image_feature)
        ...
        clip_fusion_feature = self.clip_fusion(clip_fusion_feature.float())  # torch.Size([B, 320])
        _dbg_shape("clip_image_feature", clip_image_feature)
        _dbg_shape("clip_text_feature", clip_text_feature)
        _dbg_shape("clip_fusion_feature (MLP后)", clip_fusion_feature)

        text_atn_feature = self.text_attention(text_feature, masks)
        _dbg_shape("text_atn_feature (197词→1向量)", text_atn_feature)
        image_atn_feature, _ = self.image_attention(image_feature)
        _dbg_shape("image_atn_feature (197patch→1向量)", image_atn_feature)
        ...
        _dbg_shape("fusion_feature (图文拼接序列)", fusion_feature)
        _dbg_shape("fusion_atn_feature (MLP后)", fusion_atn_feature)

        text_gate_input = text_atn_feature  # ([B, 768])
        ...
        for i in range(1):
            ...
            gate_share_expert = 0
            for j in range(self.num_expert):
                tmp_expert = self.text_experts[i][j](text_feature)  # ([B, 320])
                if _DEBUG_SHAPES and i == 0 and j == 0:
                    _dbg_shape("text_feature → TextCNN 单专家输出", tmp_expert)
        ...
        if _DEBUG_SHAPES:
            _dbg_shape("text_experts_feature (18专家加权后)", text_experts_feature)
